...
class VisageWriter:
    def __init__(self) -> None:
        env = Environment(
            loader=FileSystemLoader(
                pathlib.Path(__file__).parent.resolve() / "templates" / "visage"
            ),
            autoescape=True,
    ...
    def render_xml(self, annotationSet: "AnnotationSet") -> str:
    ...
    def volume_hash(self, datasets: "DicomVolume") -> str:
        m = hashlib.md5()
        for d in datasets:
    ...
    def generate(
        self, dcm_volume: "DicomVolume", annotation_set: AnnotationSet
    ) -> Dataset:
        ex = dcm_volume[0]
        ...
        for r in dcm_volume:
            ...
            if first:
                ...
                refd_image[(0x71, 0x1061)] = pydicom.DataElement(
                    (0x71, 0x1061), "UT", self.volume_hash(dcm_volume)
                )
